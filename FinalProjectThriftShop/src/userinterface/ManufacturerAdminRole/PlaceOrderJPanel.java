...
import Business.Manufacturer.ItemDirectory;
...
import Business.WorkQueue.ManufacturerOrderRequest;
...
public class PlaceOrderJPanel extends javax.swing.JPanel {
     ...
     */
    public PlaceOrderJPanel(JPanel userProcessContainer, UserAccount account, EcoSystem ecosystem,Organization organization) {
        initComponents();
        initListners();
        ...
        manufacturerDirectory = ecosystem.getManufacturerDirectory();
        menuTable = (DefaultTableModel) tblOrder.getModel();
        cartTable = (DefaultTableModel) tblCart.getModel();
//        populateManufacturerList(manufacturerDirectory.getManufacturerDirectory());
        if (manufacturerDirectory.getManufacturerDirectory().size() > 0) {
            index = 0;
            populateMenu();
        }
   ...
   /* public void populateManufacturerList(ArrayList<Manufacturer> manufacturersList) {
        for (Manufacturer manufacturer : manufacturersList) {
            restaurantComboBox.addItem(manufacturer.getManufacturerName());
        }
    }*/
    ...
    private void populateMenu() {
        ...
        Manufacturer manufacturer = manufacturerDirectory.getManufacturerDirectory().get(index);
        ItemDirectory itemDirectory = manufacturer.getItemDirectory();
        List<Item> items = itemDirectory.getItemList();
        for (Item item : items) {
    ...
    private boolean createOrder() {
        ...
        if (itemsWithQuantityList.isEmpty()) {
            ...
            ManufacturerOrderRequest orderWorkRequest = new ManufacturerOrderRequest();
            orderWorkRequest.setItemsWithQuatityList(itemsWithQuantityList);
            orderWorkRequest.setMessage(message.getText());
            if (manufacturer != null) {
                orderWorkRequest.setManufacturer(manufacturer);
            } else {
